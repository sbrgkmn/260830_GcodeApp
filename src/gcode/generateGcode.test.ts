...
describe('G-code generator', () => {
  it('emits coordinated XYZ extrusion moves and a safe end sequence', () => {
    ...
    expect(gcode).toContain('one continuous anchor-and-span veil')
    expect(gcode).toContain('uninterrupted helical route')
  ...
  it('matches the Creality Ender-3 V3 Plus macro and relative-extrusion contract', () => {
    ...
    expect(gcode).toContain(';SPIRAL_TURN')
    expect(gcode.match(/^G92 E0$/gm)).toHaveLength(1)
