...
export function validateToolpath(surface: ParametricSurface, toolpath: Toolpath, profile: PrinterProfile, settings: PrintSettings, _researchMode = false): ValidationResult {
  ...
  if (toolpath.jointOverlap < 0.03) issues.push({ id: 'joint-contact', severity: 'block', title: 'Anchor beads do not overlap', detail: `${toolpath.layerPitch.toFixed(2)} mm spiral pitch exceeds the conservative ${toolpath.jointBeadHeight.toFixed(2)} mm anchor-bead height. Reduce spiral pitch to restore at least 0.03 mm modeled fusion.` })
  ...
  if (toolpath.jointOverlap >= 0.03) issues.push({ id: 'joint-fusion', severity: 'info', title: 'Modeled anchor fusion', detail: `Anchor beads overlap the preceding revolution by ${toolpath.jointOverlap.toFixed(2)} mm, forming continuous structural ribs.` })

  const categoryScores = {
    continuity: toolpath.continuousPathCount === 1 ? 100 : 40,
    support: toolpath.jointOverlap >= 0.03 ? clampScore(96 - spanRisk * 12) : 20,
    collision: 96,
  ...
  return { isExportBlocked: issues.some((issue) => issue.severity === 'block'), score: clampScore(Object.values(categoryScores).reduce((a, b) => a + b, 0) / 5), categoryScores, maxVolumetricFlow, minClearance: toolpath.jointOverlap, issues }
}
